...
public class BookController {
	...
	public String borrowBook(Model model, @PathVariable("id") Long id, HttpSession session) {
		...
		// set Book Borrower : Borrower is set to user logged in in session (user):
		Book book = bookService.findBook(id);
	...
	// 2 nd One To Many : Return Book :
		...
		public String returnBook(Model model, @PathVariable("id") Long id, HttpSession session) {
			...
			// set Book Borrower to null in order to display it in the first table
			Book book = bookService.findBook(id);
